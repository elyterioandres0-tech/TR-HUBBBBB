...
function Ocean:NewWindow()
	...
	local TabHolder = createInstance("ScrollingFrame", {
		Name = "TabHolder",
		Position = UDim2.fromScale(0.000, 0.220),
	...
	local Title = createInstance("TextLabel", {
		...
		Text = '<font color="#00d4ff">TR</font> <font color="#ffffff">Hub</font>',
		TextScaled = true,
	...
	local OwnerLabel = createInstance("TextLabel", {
		Name = "OwnerLabel",
		Position = UDim2.fromScale(0.029, 0.130),
		Size = UDim2.fromScale(0.250, 0.040),
		Parent = Main,
		BackgroundTransparency = 1,
		Font = Enum.Font.Gotham,
		Text = 'Owner: <font color="#00d4ff">TR TOK</font>',
		TextScaled = true,
		TextXAlignment = Enum.TextXAlignment.Center,
		TextYAlignment = Enum.TextYAlignment.Center,
		RichText = true,
	...
	local CoFounderLabel = createInstance("TextLabel", {
		Name = "CoFounderLabel",
		Position = UDim2.fromScale(0.029, 0.170),
		Size = UDim2.fromScale(0.250, 0.040),
		Parent = Main,
		BackgroundTransparency = 1,
		Font = Enum.Font.Gotham,
		Text = 'Co-Founder: <font color="#00d4ff">ALL</font>',
		TextScaled = true,
		TextXAlignment = Enum.TextXAlignment.Center,
		TextYAlignment = Enum.TextYAlignment.Center,
		RichText = true,
	...
	function Window:Tab(Title: string, ImageID: string)
		...
		BackgroundTransparency = 1,
		Font = Enum.Font.Gotham,
		Text = 'TR Hub / <font color="#00d4ff">'..Title..'</font>',
		TextScaled = true,
		TextXAlignment = Enum.TextXAlignment.Left,
		TextYAlignment = Enum.TextYAlignment.Center,
		RichText = true,
		...
		function tab:Activate()
			...
			TS:Create(TabTitle, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {TextColor3 = Color3.fromRGB(0, 212, 255)}):Play()
			TS:Create(Icon, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {ImageColor3 = Color3.fromRGB(0, 212, 255)}):Play()
			Path.Visible = true
			Canvas.Visible = true
		end
		...
		function tab:Toggle(Title: string, Options: table)
			...
			function Settings:Toggle(v)
				...
				if Settings.Value then
					...
					TS:Create(Knob_17, TweenInfo.new(0.1, Enum.EasingStyle.Sine), {BackgroundColor3 = Color3.fromRGB(0, 212, 255)}):Play()
...
local BeOn = false
	workspace.ChildAdded:Connect(function(v)
		...
		local t2 = OceanLib:Tab("Physics", 10723424505)
		local t3 = OceanLib:Tab("Automatics", 10723426986)
		local t4 = OceanLib:Tab("Throwing", 10723426986)
		local t5 = OceanLib:Tab("Kicker", 10723426986)
		local t6 = OceanLib:Tab("UI Settings", 10723426986)
		local t7 = OceanLib:Tab("Visuals", 10723426986)
		local t8 = OceanLib:Tab("Miscellaneous", 10723426986)
...
game:GetService("RunService").RenderStepped:Connect(function()
    for index, ball in pairs(workspace:GetChildren()) do
		...
		-- Automatics Tab (t3)
		t3:Toggle("Auto Catch", {
			Default  = false,
			Callback = function(v)
				autoCatchOn = v
			end,
		})
		t3:Slider("Auto Catch Distance", {
			Default  = 10,
			Min		 = 10,
			Max		 = 20,
			Callback = function(v)
...
UIS.InputBegan:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.F then
		...
		t6:Toggle("Minimize on Start", {
			Default  = false,
			Callback = function(v)
				-- Minimize feature
			end,
		})
		t6:Toggle("Save Settings", {
			Default  = false,
			Callback = function(v)
				-- Save settings feature
			end,
		})
		t6:Toggle("Load Settings", {
			Default  = false,
			Callback = function(v)
		...
		t4:Toggle("Auto Power", {
			Default  = false,
			Callback = function(v)
				AutoPowa = v
			end,
		})
		t4:Toggle("High Power Mode", {
			Default  = false,
			Callback = function(v)
				Highestpwrmode = v
			end,
		})
		t4:Toggle("Auto Mode Selection", {
			Default  = false,
			Callback = function(v)
				autopmode = v
			end,
		})
		t4:Toggle("Auto Select WR", {
			Default  = false,
			Callback = function(v)
				autoswr = v
			end,
		...
		t4:Toggle("Q to lock on", {
			Default  = false,
			Callback = function(v)
				print("ok")
			end,
		})
		t4:Toggle("R and F to change angle manually", {
			Default  = false,
			Callback = function(v)
				print("ok")
			end,
		})
		t4:Toggle("Z and X to change power manually", {
			Default  = false,
			Callback = function(v)
				print("ok")
			end,
		})
		t4:Toggle("C to change modes", {
			Default  = false,
			Callback = function(v)
				print("ok")
			end,
		...
		-- Visuals Tab (t7)
		t7:Toggle("Magnet Hitbox Visualizer", {
			Default  = false,
			Callback = function(v)
				on3 = v
			end,
		})
		t7:Toggle("Ball Trajectory Beam", {
			Default  = false,
			Callback = function(v)
				BeOn = v
			end,
		})
		t7:Toggle("Football ESP", {
			Default  = false,
			Callback = function(v)
				-- Football ESP feature
			end,
		})
		t7:Toggle("Trajectory Visual", {
			Default  = false,
			Callback = function(v)
				-- Trajectory Visual feature
			end,
		})
